...
<template>
    <Dialog v-model:visible="props.visible" :style="{ width: '50vw' }" header="User Detail" :modal="true">
        <div v-if="user">
            ...
            <div v-if="user.roles && user.roles.length">
                ...
                <ul>
                    <li v-for="role in user.roles" :key="role.role_name">
                        <strong>{{ role.role_name }}</strong> - {{ role.role_desc }}
                    </li>
                </ul>

                <!-- Permissions per Role -->
                <div v-for="role in user.roles" :key="role.role_name">
                    <h4 class="mt-2">Permissions for {{ role.role_name }}</h4>
